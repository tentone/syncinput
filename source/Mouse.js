...
function Mouse()
{
	...
	if("ontouchstart" in window || navigator.msMaxTouchPoints > 0)
	{
		...
		document.addEventListener("touchstart", function(event)
		{
			...
			this.updateKey(this.LEFT, Key.KEY_DOWN);
		}, false);

		//Touch screen released event
		document.addEventListener("touchend", function(event)
		{
			this.updateKey(this.LEFT, Key.KEY_UP);
		}, false);
		...
		document.addEventListener("touchmove", function(event)
		{
			...
			if(this.canvas !== null)
			{
				...
				this.updatePosition(touch.clientX - rect.left, touch.clientY - rect.top, touch.clientX - last_touch.x, touch.clientY - last_touch.y);
			}
			else
			{
				this.updatePosition(touch.clientX, touch.clientY, touch.clientX - last_touch.x, touch.clientY - last_touch.y);
			}
	...
	else
	{
		...
		document.addEventListener("mousemove", function(event)
		{
			...
			{
				var rect = this.canvas.getBoundingClientRect();
				this.updatePosition(event.clientX - rect.left, event.clientY - rect.top, event.movementX, event.movementY);
			}
			else
			{
				this.updatePosition(event.clientX, event.clientY, event.movementX, event.movementY);
			}
		}, false);
		...
		{
			this.updateKey(event.which - 1, Key.KEY_DOWN);
		}, false);

		//Button released event
		document.addEventListener("mouseup", function(event)
		{
			this.updateKey(event.which - 1, Key.KEY_UP);
		}, false);
	}
}
